...
void main(void)
{
    ...
    while(1) {
        while(1) {  //wait for it to be pressed, remember current value of state
            buttonVal = state();
...
void setupIO(void){
    ...
    // Configure External Switch 2, special function bits to 0, direction to input, resistor enabled as a pull up
    P6->SEL0 &= ~BIT5;
    P6->SEL1 &= ~BIT5;
    P6->DIR &= ~BIT5;
    P6->REN |= BIT5;
...
* none
* return:
* 0 if both buttons pressed
* 1 if first button pressed
* 2 if second button pressed
* 3 if no button pressed
*************************************************************/
int state(void){
    int temp,temp2;
    temp=P6->IN & (BIT4 | BIT5); // get initial switch condition and save it
    while(1){
        SysTick_delay(15); // delay a bit
        temp2 = P6->IN & (BIT4 | BIT5); // get another switch condition
